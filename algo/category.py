...
class Category: 
    ...
    def update(self, data: UpdateInfo, dataNLP: DataNLP):
        print( '{}: intersections count before update for {} is {}'.format(TAG, self.type, self.intersections) )
        ...
        if self.type is not CategoryType.GENERAL_INFO and self.type is not CategoryType.FRIENDS:
            self.intersections = self.processor.process()
        print( '{}: intersections count after update for {} is {}'.format(TAG, self.type, self.intersections) )
